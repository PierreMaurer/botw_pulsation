interface CardProps {
  ...
  title: string;
  description: string;
  hideGrainOnHover?: boolean;
}

export default function Card({ image, header, title, description, hideGrainOnHover = false }: CardProps) {
  return (
    <div
    ...
    >
      ...
      <div
        className={`absolute inset-0 overflow-hidden rounded-[25px] mix-blend-soft-light pointer-events-none opacity-30 transition-opacity duration-300 ${hideGrainOnHover ? "group-hover:opacity-0" : ""}`}
        style={{
